...
- date: **決済日（実際に支払いをした日／領収日）** をYYYY-MM-DD形式で
  ※ 日付が複数ある場合の優先順位（必ず守ること）:
      1位: 「領収日」「お支払日」「決済日」「ご利用日」「Date of payment」「Payment date」「Transaction date」
      2位: 発行日／発行年月日／Issue date／Invoice date
      3位: レシート右上・下部に1つだけ書かれている日付（明細の日付）
  ※ 以下は**絶対に決済日として使わない**:
      - 契約開始日／契約期間／有効期限／請求期間／サービス利用期間／Next billing date／Period
      - 「〜まで」「〜から」のような期間表現の日付
